...
class gMark():
    def __init__(self, size="sf1", max_op=10000, update_ratio=0.8, delete_ratio=0.1, vt_min = 0, vt_max = 1000000, vt_ratio = 50, vt_novt_ratio = 10):
        SIZES = {
        ...
        self._vt_min = vt_min
        self._vt_max = vt_max
        self._vertex_vt = self._num_vertices * vt_ratio / 100
        self._edge_vt = self._num_edges * vt_novt_ratio/2/100 * vt_ratio/100
        self._property_vt = self._num_vertices * vt_novt_ratio/2/100 * vt_ratio/100
    ...
    def add_node_vt(self, vt_min, vt_max):
        self._id_to_be = self._id_to_be + 1
        query=f"Create (n:Node {{id:{self._id_to_be}}}) FOR VT FROM {vt_min} TO {vt_max}"
        tgql_query=f"Create (n:Object {{ title:'Node',id:{self._id_to_be},validTimeStart:timestamp(),validTimeEnd:9223372036854775000 }}) Create (n)-[:OBJECT_ATTRIBUTE]->(att:Attribute {{title:'id',validTimeStart:timestamp(),validTimeEnd:9223372036854775000}})  Create (att)-[:ATTRIBUTE_VALUE]->(:Value {{title:0,validTimeStart:timestamp(),validTimeEnd:9223372036854775000}})  Create (n)-[:OBJECT_ATTRIBUTE]->(att2:Attribute {{title:'name',validTimeStart:timestamp(),validTimeEnd:9223372036854775000}})  Create (att2)-[:ATTRIBUTE_VALUE]->(:Value {{title:0,validTimeStart:timestamp(),validTimeEnd:9223372036854775000}}) ;"
        return query,tgql_query
...
if __name__ == "__main__":
    ...
    parser.add_argument("--min-vt", type=int,
                        default=0,
                        help="The minimum valid time for data")
    parser.add_argument("--max-vt", type=int,
                        default=1000000,
                        help="The maximum valid time for data")
    parser.add_argument("--prob-vt", type=int,
                        default=50,
                        help="Probability of a node having VT features")
    parser.add_argument("--prob-vt-novt", type=int,
                        default=10,
                        help="Probability of an edge with vt linking nodes with no VT features, or setting properties to a node with no VT features")
    ...
    gmark = gMark(size=args.size, max_op=args.num_op, vt_min=args.min_vt, vt_max=args.max_vt, vt_ratio=args.prob_vt, vt_novt_ratio=args.prob_vt_novt )
